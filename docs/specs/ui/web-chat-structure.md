...
|-----------|------|
| `not_installed` | `[安装]` |
| `installed` | `[禁用]` `[钉选/取消钉选]` `[卸载]` `[📂]` |
| `update_available` | `[更新]` `[卸载]` |
| `disabled` | `[启用]` `[卸载]` |
...
| 同步状态 | 附加按钮 |
|---------|---------|
| `source_newer`（源码有更新） | `[从源码同步]` |
| `installed_newer` / `modified` / `no_source` | `[发布到源码]` |
| `conflict`（冲突） | `[用源码覆盖]` `[发布到源码]` |

**📂 目录按钮**：已安装或有源码路径时显示，下拉菜单可选"打开安装目录"或"打开源码目录"。
...
|----------|------|
| `not_installed` | `[安装]` |
| `installed` | `[运行]` `[详情]` `[收藏/取消收藏]` `[📂]`（user 来源额外：`[编辑]` `[发布]` `[删除]`） |
| `update_available` | `[运行]` |

**收藏**：收藏的 Tool 在 D1 最近使用面板置顶显示，方便快速调用。

**📂 目录按钮**：已安装时显示，点击打开 Tool 安装目录。
...
| `actions` | 安装/启用/禁用/钉选/卸载/更新/同步/发布/📂 | 安装/运行/详情/收藏/编辑/发布/删除/📂 |
| `favorited` | — | 是否已收藏 |
| `pinned` | 是否已钉选 | — |
