...
| S-a | 图标 | 按 DCC 软件区分：🎨 Blender / 🏗 Maya / 📐 Max / 🎮 Unreal / 🧩 通用 |
...
| S-g | 元信息 | 来源标签 · 版本号 · 作者 · 修改时间 |
| S-c | 操作按钮 | 根据状态和同步状态动态显示（见下方按钮表） |
...
|-----------|------|
| `not_installed` | `[详情]` `[安装]` |
| `installed` | `[详情]` `[卸载]` `[禁用]` `[钉选/取消钉选]``[收藏/取消收藏]` |
| `update_available` | `[详情]` `[卸载]` `[更新]``[禁用]``[收藏/取消收藏]` |
| `disabled` | `[详情]` `[卸载]` `[启用]` |
...
| T-g | 元信息 | 来源 · 版本 · 作者 · 修改时间 · ⚡触发规则数及类型 |
...
|----------|------|
| `not_installed` | `[详情]` `[安装]` |
| `installed` | `[详情]` `[运行]` `[收藏/取消收藏]`（user 来源额外：`[发布]` `[删除]`） |
| `update_available` | `[详情]` `[运行]` `[更新]``[收藏/取消收藏]` |

**详情界面**：点击 `[详情]` → **右侧 D 面板展开**，显示 Tool 参数输入表单 + 执行按钮（不切换整个面板，在原 D 区域内展开）。

Tool 详情面板内容：
- 完整描述、参数签名、所属 Skill 信息
- 参数输入表单（根据 manifest.inputs 动态生成：string/number/boolean/enum/select/image）
- `[执行]` 按钮 → 发送到 Chat 输入框执行
- `[打开安装目录]` 链接

**运行按钮**：点击 `[运行]` → 与详情行为一致，右侧 D 面板展开 Tool 参数表单 + 执行按钮。

**收藏**：收藏的 Tool 在 D1 最近使用面板置顶显示，方便快速调用。
...
| `meta` | 版本·作者·时间·评分·下载 | 版本·作者·评分·下载·触发规则 |
| `actions` | 详情/安装/卸载/启用/禁用/更新/发布/钉选 | 详情/安装/运行/收藏/发布/删除/更新 |
| `favorited` | — | 是否已收藏 |
| `pinned` | 是否已钉选 | — |
