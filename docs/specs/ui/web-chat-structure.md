...
| 区块 | 说明 |
|------|------|
| S-a | Skill 图标（来自 manifest 或默认图标） |
| S-b | 名称（粗体）+ 版本号（灰字） |
| S-c | 操作按钮组：启用/禁用 开关 \| 更新 \| 卸载 \| 配置齿轮 |
| S-d | Skill 描述文本（来自 SKILL.md 首段或 manifest.description） |
| S-e | 状态徽章（●启用 / ○禁用 / ⚠错误）+ "Tools: N" |
...
│ [🔍 搜索]  [按 Skill 分组 ▼]                      │
├───────────────────────────────────────────────────┤
│ C-AB2-T2: Tool 列表（按 Skill 分组折叠）            │
│                                                   │
│  ▼ blender-modeling (3 tools)                     │
│  ┌ Tool 行 ────────────────────────────────────┐  │
│  │ T-a     │ T-b                │ T-c          │  │
│  │ Tool名   │ 描述 + 参数签名    │ [调用]       │  │
│  └─────────────────────────────────────────────┘  │
...
| 区块 | 说明 |
|------|------|
| T-a | Tool 函数名（如 `create_cube`） |
| T-b | 描述（docstring 首行）+ 参数签名摘要 |
| T-c | [调用] 按钮 → 弹出参数表单 → 执行 → 显示结果 |
