...
| S-e | 来源标签：`官方` / `市集` / `用户`（对应 `00_official` / `01_team` / `02_user`） |
| 状态 | `合规`（绿色）/ `不合规`（红色），表示 Skill 是否通过 manifest 校验、依赖是否满足 |
| S-d | Skill 描述文本（来自 SKILL.md 首段或 manifest.description），1~2 行 |
| S-c | 操作按钮组：`[启用/禁用]` `[更新]` `[卸载]` `[配置]` |

**布局差异**：

| 模式 | S-a/S-b/S-e/状态 | S-d | S-c |
|------|-----------------|-----|-----|
| 卡片 | 同一行 | 单独一行（1~2 行） | 单独一行，最下方 |
| 列表 | 同一行 | 单独一行（1 行，超出省略） | 与 S-a/S-b/S-e/状态 同行，右侧 |
...
| T-e | 来源标签：`官方` / `市集` / `用户`（继承自所属 Skill） |
| 状态 | `合规` / `不合规`（继承自所属 Skill 的校验结果） |
...
| `status` | manifest 校验结果 | 继承所属 Skill |
