...
# STORY-0030 · 安装向导"检�?按钮增加部署文件校验

## 用户故事
作为用户，点击安装向导中�?检�?按钮时，不仅能看�?DCC 版本的安装状态，还能知道已安装的插件文件是否完整、是否被篡改、是否有新版本可更新�?
## 背景
- 后端已完成：`dcc_installer.py::validate_all_deployments()` + sidecar RPC `openclaw.deploy.validate`（见 `STORY-0028` 部署实录�?- 每次安装时自动在 `~/.artifexnexus/.openclaw/state/deploy-manifest.json` 记录文件 sha256 校验�?- 当前前端"检�?按钮只检查目录是否存在（`is_dcc_addon_installed()`），不做文件完整性校�?
## 验收标准
- [x] 新建 `openclaw_deploy_validate` Tauri 命令（Rust），调用 sidecar RPC `openclaw.deploy.validate`
- [x] 新建前端 IPC 函数 `validateDeployments()`，返�?`DeployValidationResult[]`
- [x] `InstallItemRow.tsx` �?DCC 父行"检�?流程末尾调用 `validateDeployments()`，结果推到日志面�?- [x] `InstallChildRow.tsx` 的子�?检�?流程同样增加校验调用
- [x] 校验结果摘要显示在日志面板：`ok N / outdated N / corrupted N / missing N`
- [x] "检�?按钮文案不变，校验为追加行为（不替代现有安装状态检测）

## 技术要�?
### 1. 新增 Tauri 命令（`apps/desktop/src-tauri/src/commands/openclaw.rs`�?```rust
#[tauri::command]
pub async fn openclaw_deploy_validate(
    sidecar: State<'_, SidecarState>,
) -> Result<serde_json::Value, String> {
    let mut manager = sidecar.lock().map_err(|e| format!("�?sidecar 失败: {e}"))?;
...
```
注册�?`lib.rs` �?`generate_handler![]` 列表�?
### 2. 注册 Tauri invoke 权限（`apps/desktop/src-tauri/capabilities/default.json`�?
### 3. 新增前端 IPC 函数（`apps/desktop/src/ipc/openclaw.ts`�?```ts
...
## 依赖
- �?[[../review/STORY-0028-gateway-mcp-bridge]]（后�?RPC `openclaw.deploy.validate` 已就绪）
- �?[[../review/STORY-0026-dcc-installer-blender]]（`validate_all_deployments()` 已实现）

## 非范�?- 不实现自动修复（corrupted �?一键重�?留到 M4�?- 不实现定时轮询校�?- 不实现通知/弹窗提醒

## 实施记录�?026-05-09�?
| 文件 | 改动 |
|------|------|
| `src-tauri/src/commands/openclaw.rs` | 新增 `openclaw_deploy_validate` Tauri 命令，调�?sidecar `openclaw.deploy.validate` |
| `src-tauri/src/lib.rs` | 注册 `openclaw_deploy_validate` �?`generate_handler![]` |
| `src/ipc/openclaw.ts` | 新增 `validateDeployments()` + `DeployValidationResult` / `DeployValidationItem` 类型 |
| `src/features/installer/InstallItemRow.tsx` | DCC 父行 `handleDetect` 末尾追加校验调用，日志输出摘�?+ 异常详情 |
