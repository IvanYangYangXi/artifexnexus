...
## 用户故事
作为 AI Agent，我能通过 WebSocket 连接到 Blender 中的 MCP Server，调用 `run_python` 工具执行 Blender Python 代码。

## 验收标准
- [ ] MCP Server 在独立线程运行 asyncio 事件循环
- [ ] 支持 MCP 协议：`initialize` / `tools/list` / `tools/call` / `ping`
- [ ] 注册 `run_python` 工具，参数 `{code: string, get_context?: boolean}`
- [ ] `get_context=true` 时返回编辑器上下文（软件/版本/选中对象/场景信息）
- [ ] 端口自动探测（8083 起始，最多探测 10 个端口）
- [ ] 单元测试覆盖 MCP 协议握手 + tools/list + tools/call

## 技术要点
- 复刻 `artclaw_bridge/subprojects/DCCClawBridge/core/mcp_server.py`
- 精简：去掉 RetryTracker、MemoryStore、SkillRuntime、Bridge UI 信号、IMAGE 标记解析
