"""
maya_ui.py - Artifex Nexus Maya 简易 UI 面板
================================================

参照 3ds Max 面板，用 PySide2 创建 Tool 窗口。
提供：状态指示、启动/停止、触发器开关。
"""
...
logger = logging.getLogger("artifex.maya.ui")
...
try:
    ...
    _HAS_QT = False
    logger.warning("PySide2 不可用，Maya UI 面板将跳过")
...
class ArtifexNexusPanel(QtWidgets.QDialog):
    ...
    def _refresh(self):
        try:
            ...
            if running:
                ...
                if clients > 0:
                    self._info_label.setText(f"端口 18081  |  已连接 ({clients})")
                else:
                    self._info_label.setText(f"端口 18081  |  等待连接...")
            else:
    ...
    def _on_start(self):
        try:
            from artifex_nexus import start_server, _shelf_start
            _shelf_start(show_alert=False)
            self._refresh()
        except Exception as e:
    ...
    def _on_stop(self):
        try:
            from artifex_nexus import stop_server
            stop_server()
            self._refresh()
        except Exception as e:
    ...
    def _on_toggle_trigger(self):
        try:
            from artifex_nexus import toggle_triggers
