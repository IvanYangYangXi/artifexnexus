...
我要在 `{{unreal|blender|maya|max|comfyui|houdini|substance-painter|substance-designer|unity}}` 中新增一个 **Skill 包** `{{skill_name}}`，作用是：
{{用一句话描述}}。
...
2. 严守命名约定：
   ...
   - **Tool = 函数**，用 `@skill_tool` 装饰（`@tool` 是推荐简写）
   - 装饰器统一 import：`from artifex_nexus.skill import skill_tool, ToolResult`（`@tool` 是推荐简写别名）
3. **不允许**新增 MCP 工具；新能力一律以 Tool 方式提供，由 `run_python` 通过 `from artifex_nexus.skill import execute; execute(name, params)` 调用。
4. 在 `packages/dcc/{{unreal|blender|maya|max|comfyui|houdini|substance-painter|substance-designer|unity}}/.../skills/{{category}}/{{skill_name}}/` 创建 Skill 包：
